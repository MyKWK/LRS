...
public class SQLite3Provider extends SQLProviderAdapter<SQLite3GlobalState, SQLite3Options> {
    ...
    public SQLConnection createDatabase(SQLite3GlobalState globalState) throws SQLException {
        String databaseName = "tpcd.db";

        File dataBase = new File(System.getProperty("user.home") + File.separator + "Desktop" + File.separator  + databaseName);

        if (!dataBase.exists()) {
            throw new SQLException("Database file " + databaseName + " does not exist.");
        }
        String url = "jdbc:sqlite:" + dataBase.getAbsolutePath();
        globalState.getState().logStatement("Connected to SQLite database: " + databaseName);
