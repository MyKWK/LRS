...
public class Rewriter {
    ...
    public Map<String, Class> rule2class = Map.of("rule_filter", Filter.class, "rule_join", Join.class, "rule_agg",
            Aggregate.class, "rule_project", Project.class, "rule_cal", Calc.class, "rule_orderby", Sort.class,
            "rule_union", Union.class);
    public List<String> rule_list = Arrays.asList("rule_agg", "rule_filter", "rule_join", "rule_project", "rule_cal",
            "rule_orderby", "rule_union");
    ...
    public Rewriter(JSONArray schemaJson) throws SQLException, IOException {
        ...
        SqlParser.Config parserConfig = SqlParser.config().withLex(Lex.MYSQL).withUnquotedCasing(UNCHANGED)
                .withCaseSensitive(false).withQuoting(DOUBLE_QUOTE).withConformance(SqlConformanceEnum.MYSQL_5);
        FrameworkConfig config = Frameworks.newConfigBuilder().parserConfig(parserConfig).defaultSchema(rootSchema)
                .build();
    ...
    public RelOptCost getCostFromRelNode(RelNode rel_node) throws Exception {
        ...
        stack.add(rel_node);
        RelOptCost tol_cost = rel_node.computeSelfCost(rel_node.getCluster().getPlanner(),
                rel_node.getCluster().getMetadataQuery());
        while (stack.size() > 0) {
            RelNode node = (RelNode) stack.pop();
            tol_cost = tol_cost
                    .plus(node.computeSelfCost(node.getCluster().getPlanner(), node.getCluster().getMetadataQuery()));
    ...
    public double getCostRecordFromRelNode(RelNode rel_node) throws Exception {
        return getCostFromRelNode(rel_node).getRows() + getCostFromRelNode(rel_node).getCpu() * 0.01
            ...
            for (int i = 0; i < node.getInputs().size(); i++) {
                ...
                if (child instanceof LogicalAggregate) {
                    if (((LogicalAggregate) child).getAggCallList().size() == 0) {
                        ...
                        if ((((String) column_names.get(flag)).contains("$"))
                                && ((column_types.get(flag).toString()).contains("BOOLEAN"))) {
                            node.replaceInput(i, child.getInput(0));
                        }
    ...
    static String get_relative_columns(List<RelNode> childs, int index) {
        ...
        if (index < childs.get(0).getRowType().getFieldNames().size()) {
            if (childs.get(0).getCluster().getMetadataQuery().getColumnOrigin(childs.get(0), index) != null) {
                res += childs.get(0).getCluster().getMetadataQuery().getColumnOrigin(childs.get(0), index)
                        .getOriginTable().getQualifiedName().get(0);
                res += "." + childs.get(0).getCluster().getMetadataQuery().getColumnOrigin(childs.get(0), index)
                        .getOriginTable().getRowType().getFieldNames().get(childs.get(0).getCluster().getMetadataQuery()
                                .getColumnOrigin(childs.get(0), index).getOriginColumnOrdinal());
            }
        } else {
            ...
            if (childs.get(1).getCluster().getMetadataQuery().getColumnOrigin(childs.get(1), index) != null) {
                res += childs.get(1).getCluster().getMetadataQuery().getColumnOrigin(childs.get(1), index)
                        .getOriginTable().getQualifiedName().get(0);
                res += "." + childs.get(1).getCluster().getMetadataQuery().getColumnOrigin(childs.get(1), index)
                        .getOriginTable().getRowType().getFieldNames().get(childs.get(1).getCluster().getMetadataQuery()
                                .getColumnOrigin(childs.get(1), index).getOriginColumnOrdinal());
            }
        }
        return res;
    }
